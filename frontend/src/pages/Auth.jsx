...
function Auth({ onLogin, onClose, initialMode = "login" }) {
  ...
  const getErrorMessage = (error, fallbackMessage) => {
    ...
    if (Array.isArray(data)) {
      const hasDuplicateUser = data.some((err) => err?.code === "DuplicateUserName");
      const hasDuplicateEmail = data.some((err) => err?.code === "DuplicateEmail");
      if (hasDuplicateUser && hasDuplicateEmail) {
      ...
      return data.map((err) => err?.description || err).join("\n");
    }
    ...
    if (Array.isArray(data.errors)) {
      const descriptions = data.errors.map((err) => err?.description || err);
      const hasDuplicateUser = data.errors.some(
        (err) => err?.code === "DuplicateUserName"
      );
      const hasDuplicateEmail = data.errors.some(
        (err) => err?.code === "DuplicateEmail"
      );
      if (hasDuplicateUser && hasDuplicateEmail) {
        return "Username và email đã tồn tại.";
      }
    ...
    if (isLogin) {
      ...
      if (
        ...
        try {
          ...
          alert(getErrorMessage(error, "Register failed. Please try again."));
        }
      }
    }
  };
              ...
              <div className="auth-feature">
                ...
                <div className="feature-text">
                  ...
                  <p>
                    ...
                    game-like features, fun challenges, and reminders.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="form-group">
                <label htmlFor="email">{isLogin ? "Username" : "Email"}</label>
                  ...
                  type={isLogin ? "text" : "email"}
                    ...
                    onChange={handleInputChange}
                  placeholder={isLogin ? "Username" : "your.email@example.com"}
                    required
                  />
                </div>
